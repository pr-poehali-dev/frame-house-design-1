...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-[var(--dark)] text-foreground font-golos">

      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/40 backdrop-blur-xl bg-[var(--dark)]/80">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <a href="#home" className="font-oswald text-2xl font-bold tracking-wider">
          ...
          <a href="#contacts" className="hidden md:inline-flex items-center gap-2 bg-neon text-[var(--dark)] font-oswald font-semibold uppercase tracking-wider px-5 py-2.5 text-sm hover:bg-neon/90 transition-colors">
            Консультация
          </a>
        ...
        {menuOpen && (
          <div className="md:hidden bg-[var(--dark2)] border-t border-border/40 px-4 py-6 flex flex-col gap-4">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="font-oswald text-lg uppercase tracking-wider hover:text-neon transition-colors"
                onClick={() => setMenuOpen(false)}
              >
                {item.label}
              </a>
            ))}
            <a href="#contacts" className="inline-flex items-center justify-center bg-neon text-[var(--dark)] font-oswald font-semibold uppercase tracking-wider px-5 py-3 mt-2">
      ...
      <section id="home" ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[var(--dark)] via-[var(--dark)]/90 to-[var(--dark)]/40" />
        ...
        <div className="container mx-auto px-4 pt-20 relative z-10">
          <div className="max-w-3xl">
            ...
            <h1
              className="font-oswald text-6xl md:text-8xl font-bold leading-none mb-6 animate-fade-up"
            ...
            <p
              className="text-lg md:text-xl text-muted-foreground max-w-xl mb-10 leading-relaxed animate-fade-up"
            ...
            >
              <a href="#portfolio" className="inline-flex items-center gap-2 bg-neon text-[var(--dark)] font-oswald font-bold uppercase tracking-wider px-8 py-4 text-base hover:bg-neon/90 transition-all hover:scale-105">
                Смотреть проекты
                <Icon name="ArrowRight" size={18} />
              </a>
              <a href="#contacts" className="inline-flex items-center gap-2 border border-neon/60 text-neon font-oswald font-bold uppercase tracking-wider px-8 py-4 text-base hover:bg-neon/10 transition-all">
                Консультация
              </a>
            </div>
          </div>
      ...
      <div className="bg-neon py-3 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          {[...TICKER_ITEMS, ...TICKER_ITEMS].map((item, i) => (
            <span key={i} className="font-oswald text-[var(--dark)] font-bold uppercase tracking-widest text-sm px-8 flex items-center gap-4">
              {item}
              <span className="inline-block w-2 h-2 rounded-full bg-[var(--dark)]/40" />
            </span>
          ))}
        </div>
      </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {PORTFOLIO.map((project) => (
              <div
                key={project.id}
                className="group relative overflow-hidden cursor-pointer card-hover bg-[var(--dark2)] border border-border/40"
              ...
              >
                <div className="aspect-[4/3] overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[var(--dark)] via-[var(--dark)]/20 to-transparent" />
                </div>

                <div className="absolute top-4 left-4 bg-neon text-[var(--dark)] font-oswald font-bold uppercase text-xs tracking-wider px-3 py-1">
                  {project.tag}
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  <h3 className="font-oswald text-2xl font-bold mb-1">{project.title}</h3>
                ...
                <div className={`absolute top-4 right-4 w-10 h-10 bg-neon flex items-center justify-center transition-transform duration-300 ${activeProject === project.id ? 'scale-100' : 'scale-0'}`}>
                  <Icon name="ArrowUpRight" size={20} className="text-[var(--dark)]" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mt-12">
            <button className="inline-flex items-center gap-2 border border-neon/60 text-neon font-oswald font-bold uppercase tracking-wider px-10 py-4 hover:bg-neon/10 transition-all">
              Все проекты
      ...
      {/* STATS BAND */}
      <div className="border-y border-border/40 bg-[var(--dark2)] py-12">
        <div className="container mx-auto px-4">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border/40">
            {SERVICES.map((service, i) => (
              <div key={i} className="bg-[var(--dark)] p-8 group hover:bg-[var(--dark2)] transition-colors relative overflow-hidden">
                <div className="absolute top-0 left-0 w-0 h-0.5 bg-neon group-hover:w-full transition-all duration-500" />
                <div className="w-12 h-12 border border-neon/40 flex items-center justify-center mb-6 group-hover:border-neon group-hover:bg-neon/10 transition-all">
                  <Icon name={service.icon} size={22} className="text-neon" />
                </div>
                <h3 className="font-oswald text-xl font-bold mb-3 group-hover:text-neon transition-colors">{service.title}</h3>
      ...
      {/* ABOUT */}
      <section id="about" className="py-24 bg-[var(--dark2)]">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <p className="text-neon font-oswald uppercase tracking-widest text-sm mb-3">— О компании</p>
              <h2 className="font-oswald text-5xl md:text-6xl font-bold mb-8 leading-tight">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="border-l-2 border-neon pl-4">
                  ...
                  <div className="text-sm text-muted-foreground mt-1">Специалистов в команде</div>
                </div>
                <div className="border-l-2 border-neon pl-4">
                  <div className="font-oswald text-3xl font-bold text-neon">200+</div>
                  <div className="text-sm text-muted-foreground mt-1">Авторских проектов</div>
                </div>
                <div className="border-l-2 border-neon pl-4">
                  <div className="font-oswald text-3xl font-bold text-neon">15</div>
                  <div className="text-sm text-muted-foreground mt-1">Регионов присутствия</div>
                </div>
              </div>
            </div>
            <div className="relative">
              ...
              <div className="absolute -bottom-6 -left-6 bg-neon p-6 w-48">
                <div className="font-oswald text-3xl font-bold text-[var(--dark)]">2014</div>
                <div className="text-[var(--dark)]/70 text-sm font-medium mt-1">Год основания компании</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
            {PROCESS.map((step, i) => (
              <div key={i} className="relative group">
                {i < PROCESS.length - 1 && (
                  <div className="hidden lg:block absolute top-10 left-[60%] right-0 h-px bg-neon/30 z-10" />
                )}
                <div className="p-8 border border-border/40 mr-0 lg:mr-px bg-[var(--dark)] group-hover:bg-[var(--dark2)] transition-colors h-full">
                  <div className="font-oswald text-6xl font-bold text-neon/20 group-hover:text-neon/40 transition-colors mb-4 leading-none">
                    {step.num}
                  </div>
                  <h3 className="font-oswald text-xl font-bold mb-3 group-hover:text-neon transition-colors">{step.title}</h3>
      ...
      <div className="bg-neon py-16 overflow-hidden relative">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <h2 className="font-oswald text-4xl md:text-6xl font-bold text-[var(--dark)] mb-4">
            ГОТОВЫ ПОСТРОИТЬ ДОМ МЕЧТЫ?
          </h2>
          <p className="text-[var(--dark)]/70 mb-8 text-lg max-w-xl mx-auto">
            Получите бесплатную консультацию и расчёт стоимости за 24 часа
          </p>
          <a href="#contacts" className="inline-flex items-center gap-3 bg-[var(--dark)] text-neon font-oswald font-bold uppercase tracking-wider px-10 py-4 text-lg hover:bg-[var(--dark2)] transition-all hover:scale-105">
            Получить расчёт
      ...
      {/* CONTACTS */}
      <section id="contacts" className="py-24 bg-[var(--dark2)]">
        <div className="container mx-auto px-4">
            ...
            <div>
              ...
              <div className="flex flex-col gap-5">
                <a href="tel:+74951234567" className="flex items-center gap-4 group">
                  <div className="w-12 h-12 border border-neon/40 flex items-center justify-center group-hover:bg-neon/10 group-hover:border-neon transition-all">
                  ...
                  <div>
                    ...
                    <div className="font-oswald text-lg font-semibold group-hover:text-neon transition-colors">+7 (495) 123-45-67</div>
                  </div>
                </a>
                <a href="mailto:info@karkas.ru" className="flex items-center gap-4 group">
                  ...
                  <div>
                    ...
                    <div className="font-oswald text-lg font-semibold group-hover:text-neon transition-colors">info@karkas.ru</div>
                  </div>
                </a>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-xs text-muted-foreground uppercase tracking-wider mb-0.5">Офис</div>
                    <div className="font-oswald text-lg font-semibold">Москва, ул. Строителей, 42</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              {formSent ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] border border-neon/40 bg-neon/5 p-12 text-center">
                  <div className="w-16 h-16 bg-neon flex items-center justify-center mb-6">
                    <Icon name="Check" size={32} className="text-[var(--dark)]" />
                  </div>
                  <h3 className="font-oswald text-3xl font-bold mb-3">Заявка отправлена!</h3>
                  <p className="text-muted-foreground">Мы свяжемся с вами в течение 30 минут в рабочее время.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="bg-[var(--dark)] border border-border/40 p-8 flex flex-col gap-5">
                  <h3 className="font-oswald text-2xl font-bold mb-2">Оставить заявку</h3>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[var(--dark2)] border border-border/60 text-foreground px-4 py-3 focus:outline-none focus:border-neon transition-colors placeholder:text-muted-foreground/50"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-[var(--dark2)] border border-border/60 text-foreground px-4 py-3 focus:outline-none focus:border-neon transition-colors placeholder:text-muted-foreground/50 resize-none"
                    />
                  </div>
                  <button
                    type="submit"
                    className="w-full bg-neon text-[var(--dark)] font-oswald font-bold uppercase tracking-wider py-4 text-base hover:bg-neon/90 transition-all hover:scale-[1.01] flex items-center justify-center gap-2"
                  >
                    Получить консультацию
                    <Icon name="ArrowRight" size={18} />
                  </button>
      ...
      {/* FOOTER */}
      <footer className="border-t border-border/40 py-8 bg-[var(--dark)]">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-4">
              {["Instagram", "Youtube", "MessageCircle"].map((icon) => (
                <button key={icon} className="w-9 h-9 border border-border/60 flex items-center justify-center hover:border-neon hover:text-neon transition-all">
